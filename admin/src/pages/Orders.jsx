...
const Orders = () => {
    ...
    const [activeTab, setActiveTab] = useState(() => {
        const savedTab = localStorage.getItem('adminLastOrderTab');
        return savedTab || 'all';
    });
    ...
    useEffect(() => {
        // ... (โค้ดกรองข้อมูล switch (activeTab) ... )
        switch (activeTab) {
            case 'pending':
    ...
    const handleTrackingChange = (orderId, value) => {
        setTrackingCodes(prev => ({ ...prev, [orderId]: value }));
    };
    const handleCarrierChange = (orderId, value) => {
        setCarriers(prev => ({ ...prev, [orderId]: value }));
    };
    ...
    const handleStatusUpdate = async (orderId) => {
        ...
        const tracking_code = trackingCodes[orderId] || '';
        const shipping_carrier = carriers[orderId] || '';

        if (newStatus === 'Shipped' && (!shipping_carrier || !tracking_code)) {
            toast.error("กรุณากรอกรหัสขนส่งและเลือกบริการขนส่ง");
            return;
        }

        try {
            await axios.post(backendUrl + '/api/order/status',
                { orderId, status: newStatus, shipping_carrier, tracking_code },
                { headers: { Authorization: `Bearer ${token}` } }
            );
                            ...
                            {order.OrderItems.map(item => (
                                <div key={item.id} className='flex items-center gap-4 text-sm'>
                                    ...
                                    <p className='flex-1 font-semibold'>
                                        {item.Product ? item.Product.name : 'สินค้าถูกลบไปแล้ว'} 
                            ...
                            <div className='border-t pt-3 flex flex-col gap-3'>
                                ...
                                {(order.order_status === 'Shipped' || order.order_status === 'Completed') && order.tracking_code && (
                                    <div className='text-xs text-gray-600 bg-blue-50 p-3 rounded-md'>
                                        ...
                                        <p>ขนส่ง: <span className='font-medium text-gray-800'>{order.shipping_carrier}</span></p>
                                        <p>รหัสขนส่ง: <span className='font-medium text-gray-800'>{order.tracking_code}</span></p>
                                    </div>
                                )}

                                {order.order_status === 'Processing' && (
                                    <div className='bg-gray-50 p-3 rounded-md flex flex-col sm:flex-row gap-3'>
                                        <div className='flex-1'>
                                            <label className='text-xs font-medium'>บริการขนส่ง</label>
                                            <select 
                                        ...
                                        <div className='flex-1'>
                                            <label className='text-xs font-medium'>รหัสขนส่งสินค้า</label>
                                            <input 
                                                type="text"
                                ...
                                <div className='flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4'>
                                    ...
                                    <div className='flex items-center gap-2 flex-wrap justify-end'>
                                        ...
                                        {/* --- 3. แก้ไข: เปลี่ยน Link เป็น button ที่ใช้ navigate --- */}
                                        <button
                                            type="button"
                                            onClick={() => navigate(`/receipt/${order.id}`)} // <-- ใช้ navigate
